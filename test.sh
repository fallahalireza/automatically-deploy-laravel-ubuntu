...
result=$(echo "SELECT user FROM mysql.user WHERE user = '$DESIRED_USERNAME';" | docker-compose exec -T mysql mysql --defaults-file=/root/laradock/.mysql.cnf  -uroot | grep -c .)
